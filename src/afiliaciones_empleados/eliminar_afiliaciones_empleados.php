...
$sentencia = $conexion->prepare('SELECT fun_softdelete_afiliaciones_empleados(?);');
$sentencia->execute([$id_usuario]);
$resultado = $sentencia->fetchColumn();
$ok = $resultado === true || $resultado === 1 || $resultado === '1' || $resultado === 't' || $resultado === 'true';

if ($ok) {
    header('Location: listar_afiliaciones_empleados.php?success=' . urlencode('Afiliación eliminada correctamente.'));
    exit();
}

header('Location: listar_afiliaciones_empleados.php?error=delete');
exit();
