...
$sentencia = $conexion->prepare('SELECT fun_softdelete_modulos(?);');
$sentencia->execute([$id_modulo]);
$resultado = $sentencia->fetchColumn();
$ok = $resultado === true || $resultado === 1 || $resultado === '1' || $resultado === 't' || $resultado === 'true';

if ($ok) {
    header('Location: listar_modulos.php?success=' . urlencode('Módulo eliminado correctamente.'));
    exit();
}

header('Location: listar_modulos.php?error=delete');
exit();
