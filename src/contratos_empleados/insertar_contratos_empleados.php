...
if (
    ...
    header('Location: forma_contratos_empleados.php?error=datos');
    exit();
}
...
$sentencia = $conexion->prepare('SELECT fun_insert_contratos_empleados(?, ?, ?, ?, ?, ?, ?);');
$sentencia->execute([
    $id_usuario,
    $id_area,
    $puesto,
...
if (stripos($resultado, 'funcion') !== false || stripos($resultado, 'correctamente') !== false) {
    header('Location: listar_contratos_empleados.php?success=' . urlencode('Contrato creado correctamente.'));
    exit();
}

header('Location: forma_contratos_empleados.php?error=' . urlencode($resultado));
exit();