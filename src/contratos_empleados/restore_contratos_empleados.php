...
$sentencia = $conexion->prepare('SELECT fun_restore_contratos_empleados(?);');
$sentencia->execute([$id_contrato]);
$resultado = $sentencia->fetchColumn();
$ok = $resultado === true || $resultado === 1 || $resultado === '1' || $resultado === 't' || $resultado === 'true';

if ($ok) {
    header('Location: listar_contratos_empleados.php?success=' . urlencode('Contrato restaurado correctamente.'));
    exit();
}

header('Location: listar_contratos_empleados.php?error=restore');
exit();
