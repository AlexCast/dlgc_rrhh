...
$sentencia = $conexion->prepare('SELECT fun_softdelete_area(?);');
$sentencia->execute([$id_area]);
$resultado = $sentencia->fetchColumn();
$ok = $resultado === true || $resultado === 1 || $resultado === '1' || $resultado === 't' || $resultado === 'true';

if ($ok) {
    header('Location: listar_area.php?success=' . urlencode('Área eliminada correctamente.'));
    exit();
}

header('Location: listar_area.php?error=delete');
exit();
