...
$sentencia = $conexion->prepare('SELECT fun_restore_operaciones(?);');
$sentencia->execute([$id_operacion]);
$resultado = $sentencia->fetchColumn();
$ok = $resultado === true || $resultado === 1 || $resultado === '1' || $resultado === 't' || $resultado === 'true';

if ($ok) {
    header('Location: listar_operaciones.php?success=' . urlencode('Operación restaurada correctamente.'));
    exit();
}

header('Location: listar_operaciones.php?error=restore');
exit();
