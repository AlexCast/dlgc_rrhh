...
if (
    ...
    header('Location: forma_operaciones.php?error=datos');
    exit();
}

$id_operacion = (int) trim($_POST['id_operacion'] ?? 0);
$id_modulo = trim($_POST['id_modulo'] ?? '');
$nombre_operacion = trim($_POST['nombre_operacion'] ?? '');

require_once __DIR__ . '/../../app/conexion.php';

$sentencia = $conexion->prepare('SELECT fun_insert_operaciones(:id_operacion, :id_modulo, :nombre_operacion);');
$sentencia->execute([
...
if (stripos($resultado, 'funcion') !== false || stripos($resultado, 'correctamente') !== false || stripos($resultado, 'Esta vaina funcionó') !== false) {
    header('Location: listar_operaciones.php?success=' . urlencode('Operación creada correctamente.'));
    exit();
}

header('Location: forma_operaciones.php?error=' . urlencode($resultado));
exit();
