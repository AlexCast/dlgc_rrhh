...
$sentencia = $conexion->prepare('SELECT fun_restore_empleados(?);');
$sentencia->execute([$id_usuario]);
$resultado = $sentencia->fetchColumn();
$ok = $resultado === true || $resultado === 1 || $resultado === '1' || $resultado === 't' || $resultado === 'true';

if ($ok) {
    header('Location: listar_empleados.php?success=' . urlencode('Empleado restaurado correctamente.'));
    exit();
}

header('Location: listar_empleados.php?error=restore');
exit();
