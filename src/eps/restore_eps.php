...
$sentencia = $conexion->prepare('SELECT fun_restore_eps(?);');
$sentencia->execute([$id_eps]);
$resultado = $sentencia->fetchColumn();
$ok = $resultado === true || $resultado === 1 || $resultado === '1' || $resultado === 't' || $resultado === 'true';

if ($ok) {
    header('Location: listar_eps.php?success=' . urlencode('EPS restaurada correctamente.'));
    exit();
}

header('Location: listar_eps.php?error=restore');
exit();
