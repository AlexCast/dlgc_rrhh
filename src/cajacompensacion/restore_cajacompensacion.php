...
$sentencia = $conexion->prepare('SELECT fun_restore_cajacompensacion(?);');
$sentencia->execute([$id_caja]);
$resultado = $sentencia->fetchColumn();
$ok = $resultado === true || $resultado === 1 || $resultado === '1' || $resultado === 't' || $resultado === 'true';

if ($ok) {
    header('Location: listar_cajacompensacion.php?success=' . urlencode('Caja de compensación restaurada correctamente.'));
    exit();
}

header('Location: listar_cajacompensacion.php?error=restore');
exit();
