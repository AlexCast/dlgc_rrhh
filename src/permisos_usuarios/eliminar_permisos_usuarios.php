...
$sentencia = $conexion->prepare('SELECT fun_softdelete_permisos_usuarios(?, ?);');
$sentencia->execute([$id_usuario, $id_operacion]);
$resultado = $sentencia->fetchColumn();
$ok = $resultado === true || $resultado === 1 || $resultado === '1' || $resultado === 't' || $resultado === 'true';

if ($ok) {
    header('Location: listar_permisos_usuarios.php?success=' . urlencode('Permiso de usuario eliminado correctamente.'));
    exit();
}

header('Location: listar_permisos_usuarios.php?error=delete');
exit();
